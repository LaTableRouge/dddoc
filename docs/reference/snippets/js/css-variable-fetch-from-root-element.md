# Récuperer des variables css de l'element html
...
```js
const prefix = 'your-variables-prefix'
      ...
      finalArr.concat(
        ...
        [...sheet.cssRules].filter(isStyleRule).reduce((propValArr, rule) => {
          const props = [...rule.style]
            ...
            .filter(([propName]) => propName.indexOf(`--${prefix}-${valuetoSearch}`) === 0)
...
if (cssBreakpoints.length) {
  cssBreakpoints.forEach((breakpoint) => {
    ...
    breakPointObject[breakpointPrefix] = breakpoint[1]
  })
}
```
